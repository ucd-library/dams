 ...
 * @description Dynamically load this app's data models from disk and merge
 * their per-model swagger docs into one spec. This is a self-hosted
 * replacement for what fin's separate `services/fin/api` microservice does
 * today, ported from the `aggie-experts` repo's `webapp/lib/models.js`
 * (same fin/fcrepo-removal pattern, adapted to reuse `@ucd-lib/fin-service-utils`'s
 * `config`/`logger` instead of aggie's own `@ucd-lib/experts-commons` package).
 */
class FinModelLoader {
  ...
  async get(model) {
    ...
    let definition = {
      ...
      info: {
        ...
        version: '1.0.0',
        description:
          'This is the Swagger documentation for the UC Davis Library Digital Collections API.',
        license: {
