...
class PageSearch extends FinEsDataModel {
  ...
  is(id, types=[], workflows=[]) {
    let workflow = workflows.find(workflow => this.bindToWorkflows.includes(workflow));
    if( workflow ) {
      return true;
    }
    return false;
  ...
  async iaSearch(searchDocument, index) {
    ...
    for( let result of results ) {
      (await this.iaSearchResultTransform(result))
        .forEach(word => transformedResults.push(word));
    }
  ...
  async _updatePage(json, pageManifest, index) {
    ...
    let pageMetadataHash = crypto.createHash('sha256')
    ...
    let results = await this.esSearch({
      ...
      query : {
        bool : {
          filter : [
            {term : {'@graph.identifier' : node.identifier}},
          ]
        }
      }
    }, {}, index);
    ...
    if( exists ) {
      ...
      if( currentNode.pageMetadataHash === pageMetadataHash ) {
        if( currentNode && currentNode.clientMedia && currentNode.clientMedia.ocrHash === pageMetadata.md5Hash ) {
  ...
  getDefaultIndexConfig(schema) {
    ...
    analysis.filter.djvu_xml = {
      ...
      ignore_case : true,
      stopwords: [
        '_english_', 'xml', 'object', 'param', 'hiddentext', 'pagecolumn', 'region',
        'paragraph', 'line', 'word'
      ]
    }
